...
N_SPLITS = 5
SUBMISSION_PATH = DATA_DIR / "submission_lightgbm_5fold.csv"
OOF_PATH = DATA_DIR / "oof_lightgbm_5fold.csv"
...
def main() -> None:
    ...
    for fold, (train_idx, valid_idx) in enumerate(folds.split(x, y), start=1):
        print(f"\nTraining LightGBM fold {fold}/{N_SPLITS}...", flush=True)
    ...
    oof = pd.DataFrame(
        {
            ID_COL: train[ID_COL],
            TARGET: y,
            "lightgbm_5fold_pred": oof_pred,
        }
    )
    oof.to_csv(OOF_PATH, index=False)
    print(f"Wrote {OOF_PATH.name} for blending/stacking.", flush=True)
